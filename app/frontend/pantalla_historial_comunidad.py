...
class PantallaHistorialComunidad(QWidget):
    ...
    def filter_by_comunidad(self):
        ...
        selected_comunidad = self.ui.Select1.currentText()
        ...
        filtered_data = self.excel_data[self.excel_data['Comunidad'] == selected_comunidad]
    ...
    def display_table_data(self, data):
        ...
        # Clear current table rows before inserting new data
    ...
    def populate_combobox(self):
        """Populate combobox with unique 'Comunidad' values from the DataFrame."""
        if self.excel_data is not None and "Comunidad" in self.excel_data.columns:
            # Ensure all values are treated as strings
            comunidades = self.excel_data["Comunidad"].dropna().astype(str).unique()  # Convert to string
            self.ui.Select1.clear()
            self.ui.Select1.addItem("Seleccionar Comunidad")  # Add a blank item for showing all data
            self.ui.Select1.addItems(sorted(comunidades, key=str))  # Sort as strings explicitly
        else:
            QMessageBox.critical(self, "Error", "No Comunidad data found or 'Comunidad' column missing.")
